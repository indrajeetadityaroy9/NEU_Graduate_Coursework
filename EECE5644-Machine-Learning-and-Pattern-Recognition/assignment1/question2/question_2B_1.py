...
from question2.visualization import visualize_3d, plot_confusion_heatmaps


def main():
    # Define prior probabilities for each class
    prior_probs = np.array([0.3, 0.3, 0.4])
    # Define the loss matrix with heavier penalty (10 units) for certain misclassifications
    loss_matrix_10 = np.array([[0, 10, 10], [1, 0, 10], [1, 1, 0]])
    ...
    decisions_10 = bayesian_classifier(samples, prior_probs, mean_vals, cov_vals, loss_matrix_10)
    # Visualize true labels vs. predicted labels in a 3D plot
    visualize_3d(samples, true_labels, decisions_10)
    # Display confusion matrix as heatmaps
    plot_confusion_heatmaps(true_labels, decisions_10)
    # Calculate the minimum expected risk based on the decisions and the true labels using the loss matrix
    risk_10 = risk(decisions_10, true_labels, loss_matrix_10)
    print(risk_10)
